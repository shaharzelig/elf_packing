...
unsigned char * read_file(char * path)
{
	...
	size_t file_size;
	if ((file_size = get_file_size(path)) == 0) {
		perror("Input file size 0.");
		return NULL;
	}
...
bool set_perm_to_program_header(Elf64_Phdr * program_header, unsigned int flags){
...
bool encrypt_buffer(unsigned char * buffer, size_t size_of_buffer, unsigned char key)
{
...
int main(int argc, char *argv[])
{
	...
	{
		perror("elf magic error.");
		return 1;
	}

	// Getting .text section of input file.
	...
	if ((output_file_path = open(argv[2], O_CREAT| O_WRONLY)) < 0)
	{
		perror("open output file");
		return 1;
	}
	
	ssize_t bytes_written = write(output_file_path, file_buffer, get_file_size(filepath));
